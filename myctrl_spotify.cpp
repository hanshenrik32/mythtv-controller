  ...
  try {
    ...
    if (conn) {
      ...
      if (strcmp(spotifytoken,"")!=0) {
        ...
        if (remove("spotify_users_playlist.txt")!=0) write_logfile(logfile,(char *) "Error remove user playlist file spotify_users_playlist.txt");
      ...
      write_logfile(logfile,(char *) "process playlist done..");
      mysql_close(conn);
    }
  }
  catch (...) {
...
int spotify_class::spotify_get_playlist(const char *playlist,bool force,bool create_playlistdb) {
  ...
  if (file_exists(playlistfilename)) {
    ...
    file_size = filestatus.st_size;                                               // get filesize
    filecreatedate=filestatus.st_mtime;                                           // get file create date
    // if file is new (< 1 day old do not process)
    if ((filecreatedate+86400<time(NULL)) || (force)) {
      file_contents = (char*) malloc(filestatus.st_size);                           // allow mem for the buffer
      ...
      if (fread(file_contents, file_size, 1, json_file ) != 1 ) {                   // read file in
      ...
      if (conn) {
        ...
        tt = 0;
        //
        // loop over songs in playlist array and update db
        //
        while(tt<antalplaylists) {
          if (stack[tt]) {
            ...
            //
            if (!(playlistexist)) {
              snprintf(sql,sizeof(sql),"insert into mythtvcontroller.spotifycontent (name,paththumb,playid,id) values ('%s','%s','%s',%d)", spotify_playlistname , stack[tt]->feed_gfx_url,playlist, 0 );
              mysql_query(conn,sql);
              res=mysql_store_result(conn);
              if (refid==0) {
                snprintf(sql,sizeof(sql),"select id from mythtvcontroller.spotifycontent where name like '%s' limit 1", spotify_playlistname);
            ...
            if (stack[tt+1]) {
              ...
              if (playlistexist==false) {              
                snprintf(sql,sizeof(sql),"insert into mythtvcontroller.spotifycontentarticles (name,paththumb,gfxfilename,player,playlistid,artist,id) values ('%s','%s','%s','%s','%s','%s',%d)", stack[tt]->feed_name , stack[tt]->feed_gfx_url,downloadfilenamelong, stack[tt]->playlisturl, playlist , stack[tt]->feed_artist , 0 );
                mysql_query(conn,sql);
        ...
        // create playlist if needed check if exist
    ...
    printf("Error loading spotify playlist file.\n");
  }
  return tt;
}
...
int spotify_class::spotify_get_available_devices() {
  ...
  if (WEXITSTATUS(curl_exitcode)==0) {
    ...
    if ( devicenr>0 ) {
      ...
      if (conn) {
        ...
        write_logfile(logfile,(char *) "Play device list is updated ok.");
      }
    }
  }
  ...
  if ( curl_exitcode == 22 ) {
    ...
    write_logfile(logfile,(char *) "Error loading device players info from spodify by api.");
  }
...
int spotify_class::spotify_get_access_token2() {
  // works
  const char *account_token_url="https://accounts.spotify.com/api/token";
  ...
  nc = mg_connect_http(&spotify_oversigt.client_mgr, ev_handler, account_token_url, "Content-Type: application/x-www-form-urlencoded\r\n", "");
...
int spotify_class::opdatere_spotify_oversigt(char *refid) {
    ...
    fprintf(stderr,"spotify loader started... \n");
    write_logfile(logfile,(char *) "spotify db loader started.");    
    conn=mysql_init(NULL);
    // Connect to database
    if (mysql_real_connect(conn, configmysqlhost,configmysqluser,configmysqlpass, database, 0, NULL, 0)) {
...
*/
        ...
        write_logfile(logfile,(char *) "No spotify data loaded from db.");
        write_logfile(logfile,(char *) "Please check that the db exist.");
      }
    ...
    write_logfile(logfile,(char *) "spotify db loader done.");    
    return(0);
}
...
int spotify_class::opdatere_spotify_oversigt_searchtxt_online(char *keybuffer,int type) {
  ...
  if (fread(file_contents, file_size, 1, json_file ) != 1 ) {
    ...
    write_logfile(logfile,(char *) "Unable to read spotify spotify_search_result content of spotify_search_result.json");
    fclose(json_file);
    free(file_contents);                                                        //
    return 1;
  }
...
void drawcover(int x, int y, int w, int h, GLuint textureId,int id,Color5 c) {
  std::string temptxt;
  glEnable(GL_TEXTURE_2D);
