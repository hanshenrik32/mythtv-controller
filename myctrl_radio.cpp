...
radiostation_class::radiostation_class() : antal(0) {
    for(int i=0;i<radiooptionsmax;i++) {
...
int radiostation_class::opdatere_radiostation_gfx(int nr,char *gfxpath) {
    ...
    try {
      ...
      write_logfile((char *) "Error update radio station db.");
    }
    return(1);
}
...
int radiostation_class::load_radio_stations_gfx() {
    ...
    try {
      ...
      write_logfile((char *) "Error loading radio station graphic icons.");
    }
    return(1);
}
...
bool radiostation_class::check_radio_online_bool() {
    ...
    try {
      ...
      write_logfile((char *) "Error connect to mysql radio station db.");
      return(0);
    }
}
...
int radiostation_class::opdatere_radio_oversigt() {
  ...
  try {
    ...
    if (mysql_real_connect(conn, configmysqlhost,configmysqluser,configmysqlpass, dbname, 0, NULL, 0)) {
      ...
      if (res) {
        while (((row = mysql_fetch_row(res)) != NULL) && (antal<maxantal)) {
          ...
          land=atoi(row[9]);
          if (antal<maxantal) {
            strncpy(myradio.station_name,row[0],stationamelength);
            strncpy(myradio.desc,row[4],statiodesclength);
            strncpy(myradio.streamurl,row[1],statiourl_homepage);
            strncpy(myradio.homepage,row[2],statiourl_homepage);
            strncpy(myradio.gfxfilename,row[5],stationamelength);
            myradio.art=art;
            myradio.kbps=kbps;
            myradio.online=online;
            myradio.land=land;
            myradio.textureId=0;
            myradio.intnr=intnr;
            stack.push_back(myradio);
            antal++;
          }
        }
        ...
        write_logfile((char *) "Failed to update radiodb, can not connect to database: mythtvcontroller Error.");
      }
    ...
    write_logfile((char *) "Error connect to radio station db.");
  }
  return(0);
}
...
//    char tmptxt1[80];
    ...
    try {
      ...
      if (mysql_real_connect(conn, configmysqlhost,configmysqluser,configmysqlpass, dbname, 0, NULL, 0)) {
        ...
        write_logfile((char *) "Failed to connect to database.");
